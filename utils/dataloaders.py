...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for offset in range(self.seq_len):
            ...
            if self.augment:
                ...
                # Apply consistent flip left-right
                if apply_fliplr:
                    img = np.fliplr(img)
                    if nl:
                        labels[:, 1] = 1 - labels[:, 1]
        ...
        # Return all images in the sequence, along with the labels from the first image (to keep compatibility)
        return seq_images_tensor, labels_seq, paths_seq, shapes_seq
    ...
    def collate_fn(batch):
        ...
        imgs_seq, labels_seq, paths_seq, shapes_seq = zip(*batch)  # Transposed
        # imgs_seq: tuple of tensors with shape [seq_len, C, H, W]
        # labels_seq: tuple of lists of tensors
        # paths_seq: tuple of lists
        # shapes_seq: tuple of lists

        batch_size = len(imgs_seq)
        seq_len = imgs_seq[0].shape[0]  # Assuming all sequences have the same length

        # Stack images to shape [batch_size, seq_len, C, H, W]
        imgs = torch.stack(imgs_seq, dim=0)

        # Flatten paths and shapes
        paths = [p for seq in paths_seq for p in seq]
        shapes = [s for seq in shapes_seq for s in seq]
        ...
        for i in range(batch_size):
            for j in range(seq_len):
                lb = labels_seq[i][j]
                if lb is not None and lb.size(0) > 0:
                    lb[:, 0] = i * seq_len + j  # Assign a unique index for each image in the batch
                    labels.append(lb)
        if len(labels):
